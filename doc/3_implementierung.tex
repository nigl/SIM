...
\end{enumerate}
Die Koordinaten entsprechen dabei den Zellennummern (von links nach rechts, bzw. unten nach oben vortlaufend durchnummeriert). Insbesondere ist eine Kreuzung ein spezielles Hinderniss,
...
werden mit Null vorbelegt. Dabei werden die Autos mit zufälliger Position und Geschwindigkeit gemäß
einer eingestellten Dichte \(\rho\) gesetzt. Dies erledigt die Funktion \code{init\_street.m}:
\begin{enumerate}
  ...
  \item Rückgabewerte
    \begin{enumerate}
      ...
      \item Vorinitialisierte Geschwindigkeitsmatrix \(V\) von Dimension \(T \times N\) \\ mit \(N := 2n+1\)
      \item Vorinitialisierte Autonummernmatrix \(W\) von Dimension \(T \times N\) 
      \item Hindernissvektor \(o\) Vektor mit Dimension \(N\) (\(o_i \neq 0\) falls in Zelle \(i\) ein
        Hinderniss steht und \(o_i = 0\) sonst)
    \end{enumerate}
\end{iterate}
